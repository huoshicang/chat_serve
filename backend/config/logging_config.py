...
def get_log_config():
    onfig_path = os.path.join(current_dir, 'logging_config.yaml')

    with open(onfig_path, 'r', encoding="utf-8") as f:
...
def setup_logger():
    ...
    # 生成带有时间戳的日志文件名
    g.server_log_filename = os.path.join(log_dir, f"{datetime.now().strftime('%Y%m%d_%H-%M')}.log")
